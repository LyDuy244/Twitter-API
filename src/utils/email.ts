...
export const sendVerifyRegisterEmail = async (toAddress: string, email_verify_token: string, template: string = verifyEmailTemplate) => {
...
export const sendForgotPasswordEmail = async (toAddress: string, forgot_password_token: string, template: string = verifyEmailTemplate) => {
  return sendVerifyEmail(toAddress, "Forgot Password", template.
    ...
    replace("{{link}}", `${envConfig.clientUrl}/verify-password?token=${forgot_password_token}`))
}